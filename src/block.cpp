...
void SuperBlock::init_super_block() {
  if (super_->s_state == FSState::UNINIT || global_options.init_flag) {
    if (global_options.init_flag) {
      DEBUG("Find uninitialized file system");
    } else {
      ERR("Find uninitialized file system, please use --init");
      abort();
    }
    ...
    // See you ultraman, someday somewhere!
    memset(super_->s_auth_string, 0, sizeof(super_->s_auth_string));
    memcpy(super_->s_auth_string, super_block_string, sizeof(super_block_string));

    // Why flush needed?
    flush();
  } else if (super_->s_state == FSState::NORMAL) {
    if (memcmp(super_->s_auth_string, super_block_string, sizeof(super_block_string))) {
      ERR("Password wrong, unauthorized access.");
      abort();
    }
    // Super block info
...
bool BlockGroup::get_inode(uint32_t index, ext2_inode** inode) {
  ...
  if (inode_table_.find(block_index) == inode_table_.end()) {
    inode_table_[block_index] = new InodeTableBlock(inode_block_offset(block_index));
  }
...
bool BlockGroup::alloc_inode(ext2_inode** inode, uint32_t* index, mode_t mode) {
  ...
  DEBUG("[BlockGroup] Alloc_inode %u", ret);
...
off_t BlockGroup::inode_block_offset(uint32_t inode_block_index) { return block_bitmap_->offset() + (inode_block_index + 1) * BLOCK_SIZE; }

off_t BlockGroup::data_block_offset(uint32_t data_block_index) {
  return block_bitmap_->offset() + (NUM_INODE_TABLE_BLOCKS + 1) * BLOCK_SIZE + data_block_index * BLOCK_SIZE;
}
}  // namespace naivefs
