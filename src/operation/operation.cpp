...
int FileStatus::write(const char* buf, size_t offset, size_t size,
  ...
  if (offset + size > isize) {
    ...
    // write is dirty
    // since get_block...memcpy(blk->get()) is not atomic (but we can assume
    // this when the number of threads is small, and cache is big although),
    size_t ret = 0;
    size_t csz = std::min(size, BLOCK_SIZE - (size_t)offset % BLOCK_SIZE);
